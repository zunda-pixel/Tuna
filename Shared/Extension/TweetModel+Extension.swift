...
extension Sweet.TweetModel {
  public init(tweet: Tweet) {
    ...
    let pubilcMetics = try? decoder.decode(Sweet.TweetPublicMetrics.self, from: tweet.publicMetrics ?? emptyData)
    let privateMetrics = try? decoder.decode(Sweet.PrivateMetrics.self, from: tweet.privateMetrics ?? emptyData)
    let organicMetrics = try? decoder.decode(Sweet.OrganicMetrics.self, from: tweet.organicMetrics ?? emptyData)
    let promotedMetrics = try? decoder.decode(Sweet.PromotedMetrics.self, from: tweet.promotedMerics ?? emptyData)
    let attachments = try? decoder.decode(Sweet.AttachmentsModel.self, from: tweet.attachments ?? emptyData)
    ...
    let entity = try? decoder.decode(Sweet.EntityModel.self, from: tweet.entitis ?? emptyData)
    ...
    self.init(id: tweet.id ?? "", text: tweet.text ?? "", authorID: tweet.authorID,
              ...
              replyUserID: tweet.replyUserID, geo: geo, publicMetrics: pubilcMetics,
