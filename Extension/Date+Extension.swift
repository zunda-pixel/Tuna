...
extension Calendar {
  func duration(from range: Range<Date>, candidates components: [Date.ComponentsFormatStyle.Field], style: DateComponentsFormatter.UnitsStyle) -> String? {
    let formatter = DateComponentsFormatter()
    formatter.unitsStyle = style
    formatter.calendar = self

    for component in components.map(\.component) {
      let dateComponents = self.dateComponents([component], from: range.lowerBound, to: range.upperBound)
